...
router.post('/', (req, res) => {
  ...
  db.query(query, [title, content, user_id], (err, results) => {
    ...
    // Fetch the complete entry including created_at
    const fetchQuery = 'SELECT * FROM entries WHERE id = ?';
    db.query(fetchQuery, [results.insertId], (fetchErr, fetchResults) => {
      if (fetchErr) {
        console.error('Error fetching created entry:', fetchErr.message);
        return res.status(500).json({ error: 'Error fetching created entry', details: fetchErr.message });
      }

      const newEntry = fetchResults[0];
      console.log('Entry created with ID:', results.insertId); // Log the newly created entry ID
      res.status(201).json(newEntry); // Return the full entry object with created_at
    });
  });
});
...
router.put('/:id', (req, res) => {
  ...
  db.query(query, [title, content, entryId], (err, results) => {
    ...
    // Fetch the complete updated entry including created_at
    const fetchQuery = 'SELECT * FROM entries WHERE id = ?';
    db.query(fetchQuery, [entryId], (fetchErr, fetchResults) => {
      if (fetchErr) {
        console.error('Error fetching updated entry:', fetchErr.message);
        return res.status(500).json({ error: 'Error fetching updated entry', details: fetchErr.message });
      }

      const updatedEntry = fetchResults[0];
      console.log(`Entry with ID: ${entryId} updated successfully`); // Log successful update
      res.status(200).json(updatedEntry); // Return the updated entry object with created_at
    });
  });
});
