...
module.exports = async (req) => {
  ...
  if (req.method === 'PUT' && id && pathname.startsWith('/api/tasks/')) {
    try {
      ...
      if (isNaN(numericId)) {
        return createResponse({ error: 'Invalid task ID' }, 400);
      }
      ...
      const updatedTask = await fastUpdate('tasks', numericId, updateData, 3000);
  ...
  if (req.method === 'DELETE' && id && pathname.startsWith('/api/tasks/')) {
    try {
      ...
      await fastDelete('tasks', numericId, 3000);
